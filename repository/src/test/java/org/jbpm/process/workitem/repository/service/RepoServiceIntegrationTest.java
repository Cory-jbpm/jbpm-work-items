...
public class RepoServiceIntegrationTest {
    ...
    private static final String SERVICE_TASK_NAME = "Archive";
    ...
    public void testAddServiceTask(){
        ...
        assertNull(resultMap.get(RepoService.UPDATED));
        ...
        assertEquals(VIMEO_NEW, resultMap.get(RepoService.UPDATED).get(0));
    }

    @Test
    public void testRemoveServiceTask() {
        Optional<RepoData> beforeRemove = repoService.getServices().stream().filter(s -> s.getName().equals(SERVICE_TASK_NAME)).findFirst();
        assertTrue(beforeRemove.isPresent());
        beforeRemove.get().setEnabled(true);
        String serviceTaskId = beforeRemove.get().getId();

        repoService.removeServiceTask(serviceTaskId);
        Optional<RepoData> afterRemove = repoService.getServices().stream().filter(s -> s.getName().equals(SERVICE_TASK_NAME)).findFirst();
